...
def read_batch(tfr_paths, img_size, batch_size=4, is_shuffle=True, reader_name="train", num_epochs=None):
    ...
    with tf.name_scope('Batch_Inputs'):
        ...
        if is_shuffle:
            batch_images, batch_centered_images = tf.train.shuffle_batch_join(data_list,
            ...
            batch_images, batch_centered_images = tf.train.batch_join(data_list,
                                                             batch_size=batch_size,
                                                             capacity=300,
                                                             enqueue_many=True,
                                                             name='%s_data_reader' % reader_name)
    return batch_images, batch_centered_images
